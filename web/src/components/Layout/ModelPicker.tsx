...
export function ModelPicker() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      const [providersRes, installedRes, currentRes, codeModelsRes] = await Promise.all([
        fetch("/api/providers"),
      ...
      // SuperGrok models served through the opencode bridge
      if (codeModelsRes.ok) {
        const cData = await codeModelsRes.json();
        for (const id of cData.models ?? []) {
          if (typeof id !== "string" || !id.startsWith("xai/") || id.includes("imagine")) continue;
          const name = id.slice("xai/".length);
          all.push({
            name,
            provider: "supergrok",
            providerName: "xAI (SuperGrok)",
            backend: "supergrok",
            active: currentBackend === "supergrok" && name === currentModel,
          });
        }
      }
        ...
        <div className="absolute top-full right-0 mt-2 w-72 rounded-xl glass border border-border shadow-2xl z-50 overflow-hidden">
          <div className="max-h-80 overflow-y-auto scrollbar-none">
            {[...grouped.entries()].map(([providerName, providerModels]) => {
              ...
              const hasActive = providerModels.some((m) => m.active);
              return (
                <div key={providerName} className="border-b border-border/50 last:border-b-0">
                  <button
                  ...
                  {isExpanded &&
                    providerModels.map((m) => (
                      <button
                        ...
                        disabled={loading}
                        className={`w-full flex items-center gap-2.5 pl-10 pr-3 py-2 text-left transition-colors ${
                          m.active
