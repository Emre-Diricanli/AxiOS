    ...
    <form onSubmit={handleSubmit} className="flex flex-col border-t border-border shrink-0">
      {codeActive && onCodeDirChange && (
        <div className="flex items-center gap-2 px-3 pt-2">
          ...
          <input
            ...
            spellCheck={false}
            className="flex-1 bg-transparent text-xs font-mono text-foreground/70 placeholder:text-muted-foreground/50 outline-none"
          />
        </div>
      )}
      <div className="flex gap-2 p-3">
        ...
        <textarea
          ...
          disabled={disabled}
          rows={1}
          className="flex-1 resize-none rounded-md field-control px-3 py-2.5 text-sm placeholder:text-muted-foreground disabled:opacity-30 transition-colors"
        />
        {showStop ? (
          <button
            type="button"
            ...
            className="w-9 h-9 rounded-md bg-destructive/15 text-destructive border border-destructive/30 flex items-center justify-center hover:bg-destructive/25 transition-colors shrink-0"
          >
          ...
          <button
            ...
            className="w-9 h-9 rounded-md bg-primary text-primary-foreground flex items-center justify-center hover:bg-primary/90 disabled:opacity-20 transition-colors shrink-0"
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
