...
//
// When running inside CasaOS, axiosd registers its API routes with the
// CasaOS-Gateway so they're accessible through the central reverse proxy.
// This is optional — axiosd works standalone without the gateway.
...
// RegisterWithGateway registers axiosd's API routes with the CasaOS-Gateway.
// It discovers the gateway URL from the runtime file or environment, then
// registers all route prefixes that axiosd serves.
//
...
func (s *Server) RegisterWithGateway(selfAddr string, gatewayCfg GatewayConfig) {
	...
	// Routes that axiosd serves and should be exposed through the gateway
	routes := []gatewayRoute{
